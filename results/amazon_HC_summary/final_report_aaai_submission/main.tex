...
in three existing RAG pipelines and in a controlled study designed to
expose its failure modes. On multi-hop QA it raises context precision
more than tenfold over gap-based Adaptive-K and improves answer Exact
Match by 23\% (relative), while reading an order of magnitude fewer
tokens. Deployment, however, forces a harder setting: a real retriever
must calibrate one global null offline and reuse it on every unseen
query. On a controlled 80K-document product-retrieval benchmark, this
shared null makes HC retrieve nothing on over 40\% of queries. We trace
the failure to a per-query scale mismatch between the global null and
each query's non-relevant scores, and correct it with a label-free
recalibration that rescales $z$-scores from observable pool statistics
while keeping one global null. The fix recovers nearly all affected
queries whose answers exist in the pool, and HC attains the best
retrieval F1 of eight competing cutoff rules, 12\% (relative) above the
strongest fixed Top-$k$. The advantage carries through to end-to-end
generation, improving answer token-F1 by 15\% over a fixed Top-20, and
holds at corpus scales up to 100{,}000 documents.
\end{abstract}
