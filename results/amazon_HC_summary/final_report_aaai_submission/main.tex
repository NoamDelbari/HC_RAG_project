...
retrieved set with Higher Criticism Thresholding (HCT), a
well-established goodness-of-fit test from the sparse signal detection
literature that converts cosine similarities into $p$-values under a
calibrated null of irrelevant query--document pairs and keeps only the
documents whose scores are too extreme to be background noise. As a
drop-in cutoff in three existing Retrieval-Augmented Generation (RAG)
pipelines, HCT raises context precision more than tenfold over
gap-based Adaptive-K on multi-hop QA and improves answer Exact Match
by 23\% (relative), while reading an order of magnitude fewer tokens.
In a controlled 80K-document product-retrieval study designed to
expose its failure mode, a single global null makes HCT retrieve
nothing on over 40\% of queries; we trace this to a per-query scale
mismatch between the null and each query's non-relevant scores, and
correct it with a label-free recalibration that rescales $z$-scores
from observable pool statistics while keeping one global null. The fix
recovers nearly all affected queries whose answers exist in the pool,
gives HCT the best retrieval F1 of eight competing cutoff rules,
12\% (relative) above the strongest fixed Top-$k$, lifts end-to-end
answer token-F1 by 15\% over a fixed Top-20, and holds at corpus
