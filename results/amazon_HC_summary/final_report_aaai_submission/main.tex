...
more than tenfold, while reading an order of magnitude fewer tokens.
Deployment, however, forces a harder setting: a real retriever must
calibrate one global null offline and reuse it on every unseen query. On
a controlled 80K-document product-retrieval benchmark with 135 queries,
this shared null makes HC return $k{=}0$ (nothing retrieved) on 55 of
them. We diagnose this as a per-query scale mismatch between the null
and each query's non-relevant scores ($r{=}0.83$ with the failure) and
correct it with a label-free recalibration that rescales $z$-scores from
observable pool statistics while keeping one global null. The fix
recovers 97\% of the fixable queries and gives HC the best retrieval F1
of eight competing cutoff rules, 12\% above the strongest fixed Top-$k$.
The advantage carries to end-to-end generation, where answer token-F1
improves by 15\% over a fixed Top-20, and holds from 726 documents up to
100{,}000.
\end{abstract}
