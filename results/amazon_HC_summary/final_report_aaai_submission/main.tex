...
%====================================================================
\begin{abstract}
Data retrieval has become a load-bearing component of modern AI
systems: retrieving too few passages leaves a reader to hallucinate or
miss the answer, while retrieving too many wastes tokens, inflates
latency, and distracts the model with irrelevant context. We view
retrieval as a sparse signal detection and identification problem:
across a vast corpus, only a few entries, if any, are relevant to a
given query, and their similarity scores must be separated from a
dense background of near-misses. We therefore propose to select the
retrieved set by a method based on Higher Criticism Thresholding
(HCT), a well-established selection tool from the sparse signal
detection literature. We apply our HCT-based method to cutoff
document relevance scores in four Retrieval-Augmented Generation
(RAG) pipelines. On multi-hop QA (HotpotQA), HCT raises context precision more than
tenfold over gap-based Adaptive-K and improves answer Exact Match by
23\% (relative), while reading an order of magnitude fewer tokens.
These integrations inherit each pipeline's own null. The sharper
test is whether HCT survives the single global null a deployed
retriever must actually rely on: one calibration for every query.
We put HCT through that test on a controlled corpus of 80{,}000
products with construction-time ground truth for every document.
The result is informative: under a naive shared null HCT is overly
conservative and returns nothing on more than 40\% of queries. The
collapse turns out to be not a weakness of the statistic but a
scale mismatch between the shared null and each query's own score
distribution, correctable by a label-free per-query rescaling that
reads only observable pool statistics at inference time, with no
additional data, no retraining, and no tuning. With the correction,
HCT attains the best retrieval F1 of eight cutoff rules on this
corpus (0.209 vs.\ 0.187 for the strongest fixed Top-$k$ we
evaluated, a 12\% relative gain), and lifts end-to-end answer
token-F1 by 15\% over a Top-20 baseline on a 100{,}000-document
electronics corpus with \texttt{gpt-5-mini}, yielding a
query-adaptive cutoff that generalizes from hundreds to 100{,}000
documents without labeled retrieval data.
\end{abstract}
...
\caption{Overview of the HC-based retrieval cutoff. \emph{Offline}, a single
\emph{global null} is calibrated from the similarity scores of irrelevant
query--document pairs. \emph{Online}, a query and corpus yield a candidate
pool of sorted similarities; the Higher Criticism cutoff maps these scores to
$p$-values under the null and selects the $k^\star=\arg\max_i \mathrm{HC}_i$
top passages, which the generator then reads. Only the cutoff box changes
across the pipelines we evaluate.}
\label{fig:overview}
\end{figure}
...
\paragraph{Adaptive retrieval cutoffs.} The default RAG interface
\citep{lewis2020rag} passes a fixed Top-$k$ set to the generator regardless of
the score distribution. Several methods instead set the cutoff from the
retrieved scores themselves. Gap-based Adaptive-K \citep{adaptivek2025} cuts at
the largest gap between consecutive sorted scores; the clustering rule CAR
\citep{xu2025car} places the boundary at a cluster edge; and Adaptive-RAG
\citep{jeong2024adaptiverag} routes each query to a retrieval strategy by a
predicted complexity class. These geometric and routing rules read the shape of
the score curve or a query-level classifier rather than testing the scores
against a calibrated background of irrelevant matches, which is the gap our
detection view targets.

\paragraph{Multiple testing and goodness-of-fit.} Casting the cutoff as
deciding which top scores are too extreme to have come from an irrelevant-score
null connects retrieval to multiple hypothesis testing. Bonferroni
\citep{bonferroni1961dunn} controls the family-wise error rate and
Benjamini--Hochberg \citep{benjamini1995fdr} the false discovery rate by
thresholding individual $p$-values, whereas Berk--Jones \citep{berkjones1979}
and Higher Criticism \citep{donohojin2004hc} aggregate the whole ordered set of
$p$-values into a goodness-of-fit deviation from uniformity. Higher Criticism
was introduced for detecting a few weak signals sparsely mixed into a large
null \citep{donohojin2004hc}, the regime we argue matches sparse-relevance
retrieval; we adopt it as a per-query cutoff and compare it against these
alternatives on a shared $p$-value construction.

\paragraph{Retrievers, frameworks, and benchmarks.} Our dense pipelines build
on BGE embeddings \citep{bgeembeddings2023} indexed for
approximate-nearest-neighbour search with FAISS \citep{johnson2019faiss}, and
register the HC cutoff as a post-processing stage in the FlashRAG toolkit
\citep{jin2025flashrag}. We evaluate across established retrieval and QA
benchmarks---BEIR and its FiQA financial-QA task \citep{thakur2021beir}, the
multi-hop HotpotQA \citep{yang2018hotpotqa} and MuSiQue
\citep{trivedi2022musique}, and the long-context aggregation benchmark
HoloBench \citep{maekawa2024holobench}---and, to probe the sparse,
variable-$K$ regime directly, our own CrossEntityQA benchmark (construction in
the supplement).
...
Algorithm~\ref{alg:hct} states the procedure step by step.

\begin{algorithm}[tb]
\caption{HCT-Ret: Higher-Criticism retrieval cutoff}
\label{alg:hct}
\textbf{Offline (once):} sample random (query, non-relevant passage) pairs;
standardise their similarities to $z$-scores; fit the empirical CDF
$\widehat{F}_0$ of this global null.\\
\textbf{Input}: query $q$, candidate-pool size $N$\\
\textbf{Parameter}: guardrail fraction $\gamma$\\
\textbf{Output}: selected passages
\begin{algorithmic}[1]
\STATE retrieve the top-$N$ passages for $q$ with scores $s_1\ge\cdots\ge s_N$
\STATE map each score to a $z$-score $z_i$
\STATE \textit{(optional, deployed setting)}\; $z_i \gets z_i\cdot
\sigma_{\mathrm{null}}/\hat{\sigma}_q$ with $\hat{\sigma}_q=\mathrm{std}\!
\left(z_{(\gamma+1:N)}\right)$ \COMMENT{label-free per-query recalibration
(Sec.~\ref{sec:amazon})}
\STATE $p_i \gets 1-\widehat{F}_0(z_i)$ \COMMENT{tail mass under the null}
\STATE sort $p_{(1)}\le\cdots\le p_{(N)}$
\FOR{$i=1$ to $\lfloor\gamma N\rfloor$}
\STATE $\mathrm{HC}_i \gets \sqrt{N}\,(i/N-p_{(i)})/\sqrt{p_{(i)}(1-p_{(i)})}$
\ENDFOR
\STATE $k^\star \gets \arg\max_{i\le\gamma N}\mathrm{HC}_i$
\STATE \textbf{return} the top-$k^\star$ passages
\end{algorithmic}
\end{algorithm}
...
% === Phase 1 ===
%====================================================================
\subsection{Integrating HC across Retrieval Pipelines}
\label{sec:phase1}
...
\subsubsection{Where adaptivity pays: sparse, variable-$K$ retrieval}
...
\subsubsection{Same answer quality, far less context}
...
\subsubsection{When relevance is scattered: FiQA}
...
% === Phase 2 ===
%====================================================================
\subsection{A Controlled Study on Amazon}
\label{sec:amazon}
...
\subsubsection{Proof of concept: the city benchmark}
...
\subsubsection{A controlled difficulty: category-and-price queries}
...
\subsubsection{Diagnosis: three mechanisms}
...
\subsubsection{The fix and what it recovers}
...
\subsubsection{Best in class, and it generalizes}
