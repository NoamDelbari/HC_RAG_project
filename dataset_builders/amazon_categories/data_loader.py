"""
Data Loader for Amazon Categories Dataset

Provides AmazonQuery dataclass and loader function compatible
...
@dataclass
class AmazonQuery:
    """A single query from the Amazon categories dataset."""
    query_id: str
    text: str
    subcategory: str
...
def load_amazon_dataset(
    output_dir: str = None,
) -> Tuple[List[AmazonQuery], Dict[str, Set[str]]]:
    """
    Load the Amazon categories dataset.

    Args:
        output_dir: Path to output directory (default: datasets/amazon_categories)
    ...
    print(f"Loaded Amazon categories dataset:")
...
def generate_gold_answers(
    ...
    For each query, collects titles from relevant docs in the subcategory
    and formats as a sorted comma-separated list.
    ...
    for query in queries:
        ...
        for doc_id in sorted(relevant_ids):
            ...
            if doc and "text" in doc:
                # Extract title: text before first ". " (title is prepended before description)
...
if __name__ == "__main__":
    queries, qrels = load_amazon_dataset()
    for q in queries[:3]:
