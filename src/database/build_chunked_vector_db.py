"""
Build CRAG Vector Database with Document Chunking

Chunks documents before embedding to handle documents that exceed
the embedding model's maximum sequence length.

Usage:
    python build_chunked_vector_db.py                           # Build with default settings
    python build_chunked_vector_db.py --chunker recursive       # Use recursive chunking
    python build_chunked_vector_db.py --chunk-size 512          # Larger chunks
    python build_chunked_vector_db.py --max-docs 5000           # Limit documents
"""
...
def main():
    parser = argparse.ArgumentParser(description="Build CRAG vector database with chunking")
    parser.add_argument(
        "--tasks",
        nargs="+",
        default=["1_2"],
    ...
    parser.add_argument(
        ...
        help="Output path for database (default: crag_chunked_vector_db)"
    )
    parser.add_argument(
        "--use-full-html",
        action="store_true",
    ...
    parser.add_argument(
        "--save-chunks-only",
        action="store_true",
        help="Only chunk and save, skip embedding (for resuming later)"
    )
    parser.add_argument(
        "--load-chunks",
        type=str,
        default=None,
        help="Load pre-chunked data from file and only do embedding"
    )

    args = parser.parse_args()

    print("\n" + "="*80)
    print("CRAG CHUNKED VECTOR DATABASE BUILDER")
    print("="*80)
    ...
    print(f"✓ Parent documents: {len(documents)}")
